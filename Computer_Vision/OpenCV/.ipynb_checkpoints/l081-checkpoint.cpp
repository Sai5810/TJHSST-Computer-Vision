...
void drawLine(int i, int j, Mat &img, vector<Mat> &vec, int width, int height) {
    line(img, Point(vec[i].at<double>(0, 0) + width / 2, vec[i].at<double>(0, 1) + height / 2),
         Point(vec[j].at<double>(0, 0) + width / 2, vec[j].at<double>(0, 1) + height / 2),
         (0, 50, 200));
}
...
void part1(int width, int height) {
    ...
    for (int n = 0; n <= 360; n += 1) {
        ...
        drawLine(0, 1, img, cube, width, height);
        drawLine(1, 2, img, cube, width, height);
        drawLine(2, 3, img, cube, width, height);
        drawLine(3, 0, img, cube, width, height);
        drawLine(4, 5, img, cube, width, height);
        drawLine(5, 6, img, cube, width, height);
        drawLine(6, 7, img, cube, width, height);
        drawLine(7, 4, img, cube, width, height);
        drawLine(0, 4, img, cube, width, height);
        drawLine(1, 5, img, cube, width, height);
        drawLine(2, 6, img, cube, width, height);
        drawLine(3, 7, img, cube, width, height);
        vid.write(img);
    }
