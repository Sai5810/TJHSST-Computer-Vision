...
void drawLine(int i, int j, Mat &img, vector<Mat> &vec, int width, int height) {
    line(img, Point(vec[i].at<double>(0, 2) + width / 2, vec[i].at<double>(0, 1) + height / 2),
         Point(vec[j].at<double>(0, 2) + width / 2, vec[j].at<double>(0, 1) + height / 2),
         (0, 50, 200));
}
...
vector<Mat> convert(Mat &eye, vector<Mat> &vec, double plane) {
    vector<Mat> conv;
    for (int i = 0; i < vec.size(); ++i) {
        double vx = vec[i].at<double>(0, 0);
        double vy = vec[i].at<double>(0, 1);
        double vz = vec[i].at<double>(0, 2);
        double ex = eye.at<double>(0, 0);
        double ey = eye.at<double>(0, 1);
        double ez = eye.at<double>(0, 2);
        double px = plane;
        conv.push_back((Mat1d(3, 1) << px, ey-(ey-vy)*(ex-px)/(ex-vx), ez-(ez-vz)*(ex-px)/(ex-vx)));
    }
    return conv;
}
...
void drawTet(Mat &img, vector<Mat> &vec, int width, int height) {
   drawLine(1, 0, img, vec, width, height);
   drawLine(1, 2, img, vec, width, height);
   drawLine(1, 3, img, vec, width, height);
   drawLine(0, 3, img, vec, width, height);
   drawLine(3, 2, img, vec, width, height);
   drawLine(2, 0, img, vec, width, height);
}
...
void part2(int width, int height, double plane) {
   ...
   for (int n = 0; n <= 360; n += 1) {
       ...
       rotate(tetra, rot);
       Mat img(height, width, CV_8UC3, Scalar(0, 0, 0));
       vector<Mat> conv = convert(eye, tetra, plane);
       drawTet(img, conv, width, height);
       vid.write(img);
   }
    coord.close();
    vid.release();
